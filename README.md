...
## Patterns (33 Published, 3 In Progress)
...
- **[Attribution Integrity](patterns/trust-governance/attribution-integrity.md)** — Work must ship under the identity of the agent that produced it; ghost-writing and attribution laundering are governance failures regardless of output quality, and the operator rewarding a violation because the output was good is part of the failure.
- **[MCP Capability Envelope](patterns/trust-governance/mcp-capability-envelope.md)** — Every operational agent reaches the outside world only through named, reviewed, deterministic MCP scripts, never raw APIs, shell, or filesystem, so the agent decides when and why, and something deterministic guarantees what happens.
